...
parser.add_argument('--id', '--server_id', dest='serverid', default="0",
                    help='Server ID to start. Default: 0')
...
class Config:
    ...
    def _get(self):
        default = {'debug': False, 'address': ['127.0.0.1', 5254],
                   'cluster': set(), 'storage': 'rafty.persist'}
        ...
        config['storage'] = "rafty.{}.persist".format(cmdline['serverid'])
        ...
        config['address'] = config['cluster'][cmdline['serverid']]
        config['cluster'] = {(v[0], int(v[1])) for k, v in config['cluster'].items()}
